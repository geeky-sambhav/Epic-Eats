...
layout = [
    [sg.Text('\t \t \t \tEpicEats: The Virtual Canteen', text_color="cyan",justification="5")],
    [sg.Text('Regular Items',text_color="pink")],
    [sg.Text("Product\tPrice\tQuantity Available\t\tQuantity Ordering", text_color="yellow")]
]

for x in lst:
    layout.append(x)
layout.append([sg.Text('Special Items', text_color="pink")])
layout.append([sg.Text("Product\t\tPrice\tQuantity Available\tQuantity Ordering", text_color="yellow")])
for x in lst_spcl:
    layout.append(x)
        ...
        for x in plain_receipes: 
            if (values[x])!='':
                if int(values[x])>int(recipes.get(x).get("stock")):
                    window["-ttt-"].update(f"{x}'s entered stock is more than available")
                    overflow=True
                    break
                else:
                    lst[x]=int(values[x])
